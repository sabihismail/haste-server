...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var pw = request.headers['pw'];
  if (pw === null || pw === undefined || pw !== this.password) {
    winston.verbose('Invalid password: ' + pw);
    response.writeHead(400, { 'content-type': 'application/json' });
    response.end(
      JSON.stringify({ message: 'You do not have permission to add new pastes.' })
    );
    return;
  } else {
    winston.verbose('Valid password.');
  }
